...
```{r Przekodowanie zmiennych na typ factor}
...
```{r Filtrowanie dla niespełnionych walidacji}
dane %>%
...
```{r Wywołanie wykresów kolumnowych}
ggplotly(wykres15)
...
```{r Wywołanie wykresów punktowych}
ggplotly(wykres28)
...
Na podstawie wykresów pudełkowych z podrozdziału 2.1 można powiedzieć, że w zmiennej Exam_Score występuje wiele wartości odstających. W tych warunkach nie będzie można skorzystać z testów parametrycznych zakładających normalność i symetrię rozkładu. Zastosowane zostaną więc testy odporne (robust). Zastosowany został test t-Studenta.
...
Na podstawie statystyki R-kwadrat można powiedzieć, że model tłumaczy 71,87% zmienności ocen uczniów z egzaminu.
